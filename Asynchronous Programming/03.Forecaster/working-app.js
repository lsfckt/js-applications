async function attachEvents() {
    ...
    getWeatherButton.addEventListener('click', async () => {
        ...
        try {
            const locations = await getInfo(url);
            const { code } = locations.find(el => el.name === input);
            const currectCondition = await getInfo(`http://localhost:3030/jsonstore/forecaster/today/${code}`);
            const threeDayForecast = await getInfo(`http://localhost:3030/jsonstore/forecaster/upcoming/${code}`);
    ...
    function createForecastDiv(forecastObj) {
        ...
        if (Array.isArray(forecast)) {
            ...
            forecast.forEach(day => {
                ...
                const conditionSymbolSpan = createHTML('span', 'symbol', weatherSymbols[condition])
                const span2 = createHTML('span', 'forecast-data', `${low}${weatherSymbols.Degrees}/${high}${weatherSymbols.Degrees}`);
        ...
        const conditionSymbolSpan = createHTML('span', 'condition symbol', weatherSymbols[condition])
